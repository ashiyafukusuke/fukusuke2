...
export default function Menu() {
  ...
  return (
    <section id="menu" className="bg-white rounded-3xl shadow-lg shadow-black/5 border-t-8 border-terracotta p-8 md:p-12 text-ink">
        ...
        <div className="space-y-8">
          {menus.map((item, idx) => (
            <motion.div 
              ...
              className={`relative p-8 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-l-4 ${item.border} overflow-hidden group transition-colors duration-300 shadow-lg
                ${item.highlight
                  ? "bg-paper ring-2 ring-olive/40"
            ...
            >
              ...
              <div className="flex-1 z-10">
                <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-3">
                  <h3 className="text-2xl font-serif font-bold text-ink tracking-wide">{item.title}</h3>
                  <span className="text-lg font-sans text-gray-600">{item.time}</span>
                  <span className={`${item.badgeColor} text-xs text-white px-2 py-1 rounded shadow-sm`}>{item.badge}</span>
                </div>
                <p className={`leading-relaxed text-sm md:text-base ${item.highlight ? "text-gray-800" : "text-gray-700"}`}>
                  {item.desc}
                </p>
              ...
              <div className="text-left md:text-right flex-shrink-0 z-10 w-full md:w-auto mt-4 md:mt-0 border-t md:border-t-0 border-gray-200 pt-4 md:pt-0">
                <p className="text-3xl font-bold text-ink md:text-right tracking-tight">{item.price}<span className="text-base font-normal ml-1">円</span></p>
                {item.note && (
